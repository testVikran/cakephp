...
function show_cart(){
    $jq.ajax({
        ...
        success: function(transport){
            if(transport == 'hide')
            {
                window.location.href = ABSOLUTE_URL+"/js_services/index";
               
            }else{
                $jq('#cart_div').html(transport);
                $jq("html, body").animate({
                    scrollTop: 0
                }, "slow");
                
                $jq("#shop_icon").show();
                $jq("#cart_count").html( $jq("#cartcnt").val());
            }  
        },
        error: function(obj){
           // alert(obj.status + "\n" + obj.statusText);
...
function replacePromo(promotionId,val_no){
    ...
    $jq.ajax({
        ...
        success: function(transport){
            if(transport == 'no_item'){
                hideCart('0');
                $jq("#shop_icon").html('<span id="cart_count"></span>');
            }
            else
            {
            show_cart();
 ...
 function addCart(promotionId,type,item_number){
    ...
    $jq.ajax({
        ...
        success: function(transport){  
            if(transport == 'fail'){
                ...
                $jq('#sp_rad').find('input[type=checkbox]:checked').removeAttr('checked');
   ...
   function renewMembershipPopup(promotionId,promotionCode){
      
    $jq.ajax({
        url: ABSOLUTE_URL+'/shopping_carts/renewMembershipPopup/'+promotionId+'/'+promotionCode,
        type: 'POST',
        success: function(transport){
        ...
        success: function(transport){
            ...
            if(arr[0] == 'expensive'){
                ...
                if(confirm('This service belongs to the same category as an item that you had earlier added to your cart. Do you want to replace it?')){
                    ...
                    if(inArray(promotionIds[0],resumeIds )){
                        addWithoutShowCart(promotionIds[0],'0','1');
                        addWithoutShowCart(promotionIds[1],'0','1');
                        showCartVsResumePopup(promotionIds[0]);
                    }else if(inArray(promotionIds[1],resumeIds )){
                        addWithoutShowCart(promotionIds[0],'0','1');
                        addWithoutShowCart(promotionIds[1],'0','1');
                        showCartVsResumePopup(promotionIds[1]);
                    }else {
                        popup('popUpDiv');
                        addCart(arr[1],'0','1');
                    }
...
function arrayCompare(a1, a2) {
   ...
   function hideCart(type){
     ...
     if(type == 1)
     popup('popUpDiv');document.getElementById('popup_go').style.display = 'none';
...
function executiveRresumePopupValidation(prod, pmid) {
    ...
    */
       if($jq('.resume').is(':checked')){
        resume_seleted = 1;
        real_name = $jq('.resume:checked').val();
    ...
    if(resume_upload!="" || resume_seleted==1)	 {
        ...
        if(resume_upload!="")
        {
            ...
            $jq.ajax({
                ...
                type: 'POST',
                data: $jq('#executive_resume').serialize(true),
                success: function(transport){
                    ...
                    if(transport == 1)
                    {
                        ...
                        $jq('#errorResume').text('Please upload resume first.');
                        $jq('#errorResume').show();
                    }
                },
                error: function(obj){
                  ...
                  if(data == '1') {
				    isResumeUploaded=true;
                                    ...
                                    show_cart();
                                       ...
                                       $jq('#errorResume').text('Resume should be .doc, .docx files; upto 500 KB each.');
                                                 $jq('#errorResume').show();
					return false;
...
function hhresumeCritiqueValidation(promotionId,type)
{
    ...
    if($jq('#rc_availed_resume').val()!= "" && ($jq('#rc_availed_resume').val() == resume_upload || $jq('#rc_availed_resume').val() == real_name)){
    ...
    if(resume_upload!="" || resume_seleted==1)     {
        ...
        if(resume_upload!="")
        {
            ...
            $jq.ajax({
                ...
                type: 'POST',
                data: $jq('#hh_resume_critique').serialize(true),
                success: function(transport){
                    if(transport == 1)
                    {
                        popup('popUpDiv');
...
function checkMemProm(chkId){
        ...
        if(JS_STATUS==5){
            ...
            if($jq('#'+chkId).is(':checked')){
            if(chkId == 'offer15'){
                 if($jq('#salaryCheck').val()=='inSalaryScale'){
                  addWithoutShowCart('104','0','1');
                  addResumeProductCart('15','0','1');
				 }
				 else if($jq('#salaryCheck').val()=='underSalaryScale'){
...
function confirmBoxCombo(combo){
    $jq.ajax({
        ...
        type: 'POST',
        success: function(transport){
			$jq('#confirmPopup').html(transport);
        },
        error: function(obj){
           // alert(obj.status + "\n" + obj.statusText);
